...
class V32Activity : AppCompatActivity() {
    ...
    private fun field(h:String,num:Boolean=false,max:Int=0)=EditText(this).apply{hint=h;textSize=16f;setSingleLine(true);setPadding(dp(12),dp(8),dp(12),dp(8));background=box(Color.WHITE,Color.rgb(215,222,232),14);if(num)inputType=InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL;if(max>0)filters=arrayOf(InputFilter.LengthFilter(max))}
    ...
    private fun sha256(v:String)=java.security.MessageDigest.getInstance("SHA-256").digest(v.toByteArray(Charsets.UTF_8)).joinToString(""){"%02x".format(it)}
    ...
    private fun validIfsc(v:String)=v.isBlank() || Regex("[A-Z]{4}0[A-Z0-9]{6}").matches(v.trim().uppercase(Locale.US))
    private fun validAccount(v:String)=v.isBlank() || Regex("[0-9]{6,20}").matches(v.trim())
    private fun validUpi(v:String)=v.isBlank() || Regex("[A-Za-z0-9._-]{2,}@[A-Za-z]{2,}").matches(v.trim())
    private fun validEmail(v:String)=v.isBlank() || android.util.Patterns.EMAIL_ADDRESS.matcher(v.trim()).matches()
    private fun maskPan(v:String)=if(v.length==10) v.take(2)+"******"+v.takeLast(2) else if(v.isBlank()) "—" else "••••••••••"
    private fun maskAadhaar(v:String)=if(v.length==12) "••••••"+v.takeLast(6) else if(v.isBlank()) "—" else "••••••••••••"
    ...
    private fun registerCredit(preselected:Long?){
        ...
        r.addView(button("FINAL VERIFY + REGISTER CREDIT",green){
            try{
                ...
                val id=db.addCredit(b.rowId,gid,t,direction.selectedItem.toString(),a,rate,method.selectedItem.toString(),inst,interest,payable,st,en,if(t=="Trade Credit"||t=="Advance")en else st,gst.text.toString().trim().uppercase(Locale.US),invoiceUri?.toString()?:"",false);if(id<=0L){toast("Credit could not be saved");return@button}
                val scheduleCount=when(method.selectedItem.toString()){"Single Payment","Trade Invoice Due"->1;else->n.coerceIn(1,240)}
                db.makeSchedule(id,t,st,en,inst,scheduleCount,dueDay.text.toString().toIntOrNull()?:0)
                val docText="UDHAARDAAR CREDIT ACKNOWLEDGEMENT / KFS\nCredit ID: "+id+"\nBorrower: "+b.name+"\nNature: "+t+"\nDirection: "+direction.selectedItem+"\nPrincipal: "+money(a)+"\nAnnualised ROI: "+String.format(Locale.US,"%.2f",rate)+"%\nRepayment method: "+method.selectedItem+"\nInstallment: "+money(inst)+"\nTotal interest: "+money(interest)+"\nTotal payable: "+money(payable)+"\nStart: "+st+"\nEnd: "+en+"\nDigital consent: VERIFIED\nData sharing: OFF unless separately consented"
                runCatching{db.saveDocument(id,"CREDIT_KFS_ACKNOWLEDGEMENT","Credit KFS / acknowledgement",sha256(docText),invoiceUri?.toString()?:"",true)}
                toast("Credit registered successfully");creditDetail(id)
    ...
    private fun creditDetail(id:Long){val c=db.credit(id)?:return;val r=page("Credit ${c.code}","Full transaction / verification record");r.addView(text("Borrower: ${c.borrowerName}",19f,navy));r.addView(text("Nature: ${c.type}"));r.addView(text("Guarantor ID: ${c.guarantorId?:"Not provided"}"));r.addView(text("Direction: ${c.direction}"));r.addView(text("Amount: ${money(c.amount)}"));r.addView(text("ROI: ${String.format(Locale.US,"%.2f",c.roi)}%"));r.addView(text("Method: ${c.method}"));r.addView(text("Installment: ${money(c.installment)}"));r.addView(text("Interest: ${money(c.interest)}"));r.addView(text("Total payable: ${money(c.payable)}"));r.addView(text("Start: ${c.start} • End: ${c.end}"));r.addView(text("Due: ${c.due}"));if(c.type=="Trade Credit")r.addView(text("GSTIN: ${c.gstin.ifBlank {"—" } }"));r.addView(text("Bank/NACH: ${if(c.nach) "Linked" else "Not linked"}"));r.addView(text("OTP / digital consent: VERIFIED",14f,green));r.addView(text("Digital documents recorded: ${db.documentCount(id)}",13f,green));if(c.invoice.isNotBlank())r.addView(button("OPEN INVOICE",teal){startActivity(Intent(Intent.ACTION_VIEW,Uri.parse(c.invoice)).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)) });r.addView(button("VIEW / SHARE DOCUMENT",amber){shareCredit(c)});r.addView(button("REPAYMENT SCHEDULE",green){repaymentFor(id)})
        r.addView(button("MARK DEFAULT / DEFAULT CASE",red){
            val reason=field("Reason for default / classification")
            val d=AlertDialog.Builder(this).setTitle("Mark credit as DEFAULT").setMessage("This changes the credit status and marks unpaid schedules as DEFAULT.").setView(reason).setNegativeButton("CANCEL",null).setPositiveButton("CONFIRM",null).create()
            d.setOnShowListener{d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
                if(reason.text.toString().trim().length<5){reason.error="Please enter a reason";return@setOnClickListener}
                runCatching{db.markDefault(id,reason.text.toString());d.dismiss();toast("Credit marked as DEFAULT");creditDetail(id)}.onFailure{toast("Default status could not be recorded")}
            }}
            d.show()
        })
        r.addView(button("BACK",Color.DKGRAY){history(null)}); show(r)}
    ...
    private fun bankForm(type:String,id:String){val r=page("Bank / NACH","Optional linkage • do not make this mandatory");val h=field("Account holder name");val bank=field("Bank name");val acc=field("Account number",true);val ifsc=field("IFSC");val upi=field("UPI ID");val nach=CheckBox(this).apply{text="Enable NACH"};listOf(h,bank,acc,ifsc,upi).forEach{r.addView(it,LinearLayout.LayoutParams(-1,dp(58)).apply{setMargins(0,dp(3),0,dp(3))})};r.addView(nach);r.addView(button("SAVE BANK / NACH",green){val av=acc.text.toString().trim();val iv=ifsc.text.toString().trim().uppercase(Locale.US);val uv=upi.text.toString().trim();if(!validAccount(av)){acc.error="Account number must be 6-20 digits";return@button};if(!validIfsc(iv)){ifsc.error="Invalid IFSC";return@button};if(!validUpi(uv)){upi.error="Invalid UPI ID";return@button};db.saveBank(type,id,h.text.toString().trim(),bank.text.toString().trim(),av,iv,uv,nach.isChecked);toast("Optional bank/NACH link saved");dashboard()});r.addView(button("BACK",Color.DKGRAY){dashboard()}); show(r)}

    private fun documents(){val r=page("Digital Documents","Standard T&C and consent documentation");r.addView(text("Udhaardaar V3.2.6 Standard Lending Terms",20f,navy));r.addView(text("1. Parties and identity are recorded before registration.\n2. Credit nature determines applicable repayment fields.\n3. Borrower receives the digital acknowledgement before OTP consent.\n4. Repayments are recorded against the registered schedule.\n5. Trade credit may include GSTIN and invoice evidence.\n6. Bank/NACH linkage is optional.\n7. Users should retain copies of all documents and comply with applicable law."));r.addView(button("SHARE STANDARD T&C",amber){shareTerms()});r.addView(button("BACK",Color.DKGRAY){dashboard()}); show(r)}
    private fun shareTerms(){val i=Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,"Udhaardaar Standard Lending Terms V3.2.6");putExtra(Intent.EXTRA_TEXT,"UDHAARDAAR — KEY FACT / CREDIT DOCUMENT\n\nBorrower identity: recorded and verified before registration\nCredit nature: disclosed\nPrincipal, annualised interest/APR, tenor and repayment schedule: disclosed before consent\nFees/charges: must be disclosed before consent\nRecovery mechanism: must be disclosed before consent\nGrievance contact: configure before production use\nCooling-off/look-up period: configure according to the applicable lender/regulatory framework\nData sharing: OFF unless the user gives explicit consent\n\nThis trial build is a record-keeping prototype and is not a substitute for a regulated lender’s final KFS, sanction letter, loan agreement or legally executed digital signature.")};startActivity(Intent.createChooser(i,"Share T&C"))}
    private fun shareCredit(c:V32DatabaseHelper.Credit){
        val consent=CheckBox(this).apply{text="I confirm I have the required consent/legal basis to share this borrower credit information."}
        val box=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(20),0,dp(20),0);addView(consent)}
        val d=AlertDialog.Builder(this).setTitle("Privacy confirmation").setMessage("No borrower or third-party credit information is shared unless the required consent/legal basis exists. Only continue after confirming authorization.").setView(box).setNegativeButton("CANCEL",null).setPositiveButton("CONTINUE",null).create()
        d.setOnShowListener{d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
            if(!consent.isChecked){consent.error="Consent confirmation required";return@setOnClickListener}
            val i=Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,"Udhaardaar Credit ${c.code}");putExtra(Intent.EXTRA_TEXT,"Udhaardaar Credit Record\nCode: ${c.code}\nBorrower: ${c.borrowerName}\nNature: ${c.type}\nDirection: ${c.direction}\nAmount: ${money(c.amount)}\nROI: ${String.format(Locale.US,"%.2f",c.roi)}%\nMethod: ${c.method}\nInstallment: ${money(c.installment)}\nTotal payable: ${money(c.payable)}\nStart: ${c.start}\nEnd: ${c.end}\nDigital consent: VERIFIED")};startActivity(Intent.createChooser(i,"Share credit document"));d.dismiss()
        }}
        d.show()
    }
