...
class V32Activity : AppCompatActivity() {
    ...
    private fun ownerRegistration(){
        ...
        r.addView(button("CREATE PROFILE + SEND OTP",blue){if(name.text.toString().trim().length<2||mobile.text.toString().length!=10||address.text.toString().trim().length<5){toast("Name, address and exactly 10-digit mobile are required");return@button};otp=Random.nextInt(100000,1000000).toString();otpBox.visibility=View.VISIBLE;toast("Trial OTP: $otp\nConnect an SMS provider for live delivery.")})
    ...
    private fun registerCredit(preselected:Long?){
        ...
        val gs=db.profiles("GUARANTOR");val g=Spinner(this);g.adapter=ArrayAdapter(this,android.R.layout.simple_spinner_dropdown_item,listOf("Select guarantor")+gs.map{it.name+" • "+it.mobile+" • "+it.id});g.visibility=View.GONE;r.addView(g)
        ...
        r.addView(button("FINAL VERIFY + REGISTER CREDIT",green){
            try{
                ...
                val st=start.text.toString().ifBlank {today()};val en=end.text.toString().ifBlank {if(t=="Rental / Lease")dateAfter(st,n) else st};val interest=if(t=="Rental / Lease")0.0 else a*rate/100.0*n/12.0;val payable=if(t=="Rental / Lease")a*n else a+interest;val inst=if(t=="Trade Credit"||t=="Advance")payable else if(t=="Rental / Lease")a else payable/n.coerceAtLeast(1)
                ...
                val id=db.addCredit(b.rowId,t,direction.selectedItem.toString(),a,rate,method.selectedItem.toString(),inst,interest,payable,st,en,if(t=="Trade Credit"||t=="Advance")en else st,gst.text.toString().trim().uppercase(Locale.US),invoiceUri?.toString()?:"",false)
        ...
        });r.addView(button("BACK",Color.DKGRAY){dashboard()}); setContentView(r)
    }
    ...
    private fun creditDetail(id:Long){val c=db.credit(id)?:return;val r=page("Credit ${c.code}","Full transaction / verification record");r.addView(text("Borrower: ${c.borrowerName}",19f,navy));r.addView(text("Nature: ${c.type}"));r.addView(text("Direction: ${c.direction}"));r.addView(text("Amount: ${money(c.amount)}"));r.addView(text("ROI: ${String.format(Locale.US,"%.2f",c.roi)}%"));r.addView(text("Method: ${c.method}"));r.addView(text("Installment: ${money(c.installment)}"));r.addView(text("Interest: ${money(c.interest)}"));r.addView(text("Total payable: ${money(c.payable)}"));r.addView(text("Start: ${c.start} • End: ${c.end}"));r.addView(text("Due: ${c.due}"));if(c.type=="Trade Credit")r.addView(text("GSTIN: ${c.gstin.ifBlank {"—" } }"));r.addView(text("Bank/NACH: ${if(c.nach) "Linked" else "Not linked"}"));r.addView(text("OTP / digital consent: VERIFIED",14f,green));if(c.invoice.isNotBlank())r.addView(button("OPEN INVOICE",teal){startActivity(Intent(Intent.ACTION_VIEW,Uri.parse(c.invoice))) });r.addView(button("VIEW / SHARE DOCUMENT",amber){shareCredit(c)});r.addView(button("REPAYMENT SCHEDULE",green){repaymentFor(id)});r.addView(button("BACK",Color.DKGRAY){history(null)}); setContentView(r)}
    ...
    private fun shareTerms(){val i=Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,"Udhaardaar Standard Lending Terms V3.2.2");putExtra(Intent.EXTRA_TEXT,"Udhaardaar Standard Lending Terms\n\nIdentity, credit nature, repayment terms, digital acknowledgement and OTP consent are recorded. Bank/NACH linkage is optional. Trade credit may include GSTIN and invoice evidence.\n\nPlease review and retain this document.")};startActivity(Intent.createChooser(i,"Share T&C"))}
    ...
    private fun pick(code:Int){startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply{type="image/*";addCategory(Intent.CATEGORY_OPENABLE);putExtra(Intent.EXTRA_ALLOW_MULTIPLE,false)},code)}
    override fun onActivityResult(request:Int,result:Int,data:Intent?){super.onActivityResult(request,result,data);if(result!=Activity.RESULT_OK||data?.data==null)return;val u=data.data!!;if(request==100){photoUri=u;photoTarget?.setImageURI(u)} else if(request==200){invoiceUri=u;invoiceTarget?.text="Invoice selected: ${u.lastPathSegment?:"image"}" } }
}