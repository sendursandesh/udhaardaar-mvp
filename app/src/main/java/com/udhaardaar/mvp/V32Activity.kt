...
class V32Activity : AppCompatActivity() {
    ...
    private fun shareCredit(c:V32DatabaseHelper.Credit){AlertDialog.Builder(this).setTitle("Privacy confirmation").setMessage("This action will share the credit record with another app/service selected by you. Continue only if you have the required consent and legal basis.").setNegativeButton("CANCEL",null).setPositiveButton("CONTINUE"){_,_->val i=Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,"Udhaardaar Credit ${c.code}");putExtra(Intent.EXTRA_TEXT,"Udhaardaar Credit Record\nCode: ${c.code}\nBorrower: ${c.borrowerName}\nNature: ${c.type}\nDirection: ${c.direction}\nAmount: ${money(c.amount)}\nROI: ${String.format(Locale.US,"%.2f",c.roi)}%\nMethod: ${c.method}\nInstallment: ${money(c.installment)}\nTotal payable: ${money(c.payable)}\nStart: ${c.start}\nEnd: ${c.end}\nDigital consent: VERIFIED")};startActivity(Intent.createChooser(i,"Share credit document"))}}.show()}
