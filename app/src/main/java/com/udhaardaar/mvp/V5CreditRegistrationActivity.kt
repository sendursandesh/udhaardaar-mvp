...
class V5CreditRegistrationActivity : AppCompatActivity() {
    ...
    private fun e(h:String,max:Int=0)=EditText(this).apply{hint=h;setSingleLine(true);setPadding(16,8,16,8);if(max>0)filters=arrayOf(android.text.InputFilter.LengthFilter(max))}
    ...
    private fun show(){
        ...
        add(r,btn("CONFIRM & REGISTER CREDIT"){register()});add(r,btn("HOME"){goHome()});add(r,btn("BACK"){finish()});setContentView(ScrollView(this).apply{addView(r)})
    }
    private fun prepareDocuments(){
        ...
        store.replace("documents",JSONObject().apply{put("id","DPN-$id");put("type","Demand Promissory Note");put("creditId",id);put("content",V5GuarantorAndDocuments.generateDpnTemplate(id,b,l,a,r,start.text.toString(),end.text.toString()));put("status","DRAFT");put("createdAt",System.currentTimeMillis())});dpnCreated=true;guaranteeCreated=false
