...
/** Frozen ArthSaathi V6.2 visual language and approved journey logo usage. */
object ArthSaathiV62Design {
    ...
    const val BRAND="ArthSaathi"; const val TAGLINE="Navigate Your Financial Journey"; const val PILLARS="Plan • Protect • Grow • Nominate"; const val LOGO_RESOURCE="@drawable/udhaardaar_logo"
    ...
    fun title(c:android.content.Context,name:String,subtitle:String):LinearLayout{val d=c.resources.displayMetrics.density;val h=LinearLayout(c).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(dp(8,d),dp(6,d),dp(8,d),dp(6,d))};h.addView(ImageView(c).apply{setImageResource(R.drawable.udhaardaar_logo);scaleType=ImageView.ScaleType.CENTER_INSIDE;contentDescription="ArthSaathi approved journey logo"},LinearLayout.LayoutParams(dp(52,d),dp(52,d)));val b=LinearLayout(c).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(10,d),0,0,0)};b.addView(text(c,name,20f,NAVY,true));b.addView(text(c,subtitle,9.5f,TEAL,true),LinearLayout.LayoutParams(-1,-2).apply{topMargin=dp(3,d)});h.addView(b,LinearLayout.LayoutParams(0,-2,1f));return h}
