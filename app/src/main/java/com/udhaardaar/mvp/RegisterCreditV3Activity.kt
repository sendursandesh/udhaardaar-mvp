...
class RegisterCreditV3Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        partySpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onNothingSelected(parent: AdapterView<*>?) = Unit
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) { if (position in partyIds.indices) showHistory(partyIds[position]) }
        }
        listOf(amount, roi, repayment, invoice, notes).forEach { field -> field.setOnFocusChangeListener { v, hasFocus -> if (hasFocus) bringFieldAboveIme(v) } }
    ...
    private fun bringFieldAboveIme(view: View) {
        view.postDelayed({
            val rect = android.graphics.Rect(); scroll.getWindowVisibleDisplayFrame(rect)
            val location = IntArray(2); view.getLocationInWindow(location)
            val delta = location[1] + view.height - (rect.bottom - 32)
            if (delta > 0) scroll.smoothScrollBy(0, delta + 48)
        }, 120)
    }

    private fun setSpinner(spinner: Spinner, values: Array<String>) { spinner.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, values) }

    private fun loadParties() {
        partyIds.clear(); val names = mutableListOf<String>()
        db.readableDatabase.rawQuery("SELECT id,name FROM parties ORDER BY name COLLATE NOCASE", null).use { c -> while (c.moveToNext()) { partyIds.add(c.getLong(0)); names.add(c.getString(1)) } }
    ...
    private fun showHistory(partyId: Long) {
        ...
        db.readableDatabase.rawQuery("SELECT c.principal_amount,c.status,COALESCE((SELECT SUM(r.amount) FROM repayments r WHERE r.credit_id=c.id),0) FROM credits c WHERE c.party_id=? ORDER BY c.start_date DESC", arrayOf(partyId.toString())).use { c -> while (c.moveToNext()) { count++; principal+=c.getDouble(0); repaid+=c.getDouble(2); if(c.getString(1)=="ACTIVE") active++ else if(c.getString(1)=="SETTLED") settled++ } }
    ...
    private fun pickDate(target: EditText) { val c=Calendar.getInstance(); DatePickerDialog(this,{_,y,m,d->c.set(y,m,d);target.setText(df.format(c.time));bringFieldAboveIme(target)},c.get(Calendar.YEAR),c.get(Calendar.MONTH),c.get(Calendar.DAY_OF_MONTH)).show() }

    private fun calculateRepayment() {
        val p=amount.text.toString().toDoubleOrNull()?:return; val r=roi.text.toString().toDoubleOrNull()?:0.0; val method=methodSpinner.selectedItem?.toString()?:"EMI"; if(p<=0||r<0)return
        if(method=="EMI"){val monthly=r/1200.0;val emi=if(monthly==0.0)p/12.0 else p*monthly*Math.pow(1+monthly,12.0)/(Math.pow(1+monthly,12.0)-1);repayment.setText(String.format(Locale.US,"%.2f",emi));calculation.text="Indicative EMI: ${money(emi)} per month. Review against agreed terms before registering."}else{val interest=p*r/100.0;calculation.text="Principal: ${money(p)}\nIndicative annual interest at ${String.format(Locale.US,"%.2f",r)}%: ${money(interest)}\nEnter the agreed repayment amount."}
    }
    ...
    private fun validateAndSave() {
        ...
        if (!ConsentOtpGate.isRegistrationAuthorised()) return toast("OTP verification is required before registering this credit")
        val p=amount.text.toString().toDoubleOrNull()?:return field(amount,"Enter a valid amount")
        val rate=roi.text.toString().toDoubleOrNull()?:0.0; val repay=repayment.text.toString().toDoubleOrNull()?:return field(repayment,"Enter the agreed repayment amount")
        if(p<=0)return field(amount,"Amount must be greater than zero");if(rate<0||rate>100)return field(roi,"ROI must be between 0% and 100%");if(repay<=0)return field(repayment,"Repayment must be greater than zero");if(start.text.isNullOrBlank())return field(start,"Select the start date");if(end.text.isNotBlank()&&!validDateOrder(start.text.toString(),end.text.toString()))return field(end,"End date must be on/after start date")
        val type=typeSpinner.selectedItem.toString();val direction=if(directionSpinner.selectedItemPosition==0)"GIVEN" else "RECEIVED";val method=methodSpinner.selectedItem.toString().uppercase(Locale.getDefault()).replace(" + ","_").replace(" ","_");val periodicity=periodicitySpinner.selectedItem.toString().uppercase(Locale.getDefault());val now=SimpleDateFormat("yyyy-MM-dd HH:mm:ss",Locale.getDefault()).format(Calendar.getInstance().time)
        ...
        if(id>0){ConsentOtpGate.clearAfterRegistration();toast("Credit registered successfully");finish()}else toast("Unable to register credit. Nothing was saved.")
    }

    private fun validDateOrder(a:String,b:String)=try{!df.parse(b).before(df.parse(a))}catch(_:Exception){false}
