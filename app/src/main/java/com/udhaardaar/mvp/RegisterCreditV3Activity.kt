...
class RegisterCreditV3Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        amount.setOnFocusChangeListener { v, hasFocus -> if (hasFocus) { calculateRepayment(); bringFieldAboveIme(v) } }
        roi.setOnFocusChangeListener { v, hasFocus -> if (hasFocus) { calculateRepayment(); bringFieldAboveIme(v) } }
        methodSpinner.onItemSelectedListener = simpleListener { calculateRepayment() }
        periodicitySpinner.onItemSelectedListener = simpleListener { calculateRepayment() }
        partySpinner.onItemSelectedListener = simpleListener {
            if (partySpinner.selectedItemPosition in partyIds.indices) showHistory(partyIds[partySpinner.selectedItemPosition])
        }
    ...
    private fun showHistory(partyId: Long) {
        ...
        db.readableDatabase.rawQuery(
            "SELECT c.principal_amount,c.status,COALESCE((SELECT SUM(r.amount) FROM repayments r WHERE r.credit_id=c.id),0) FROM credits c WHERE c.party_id=? ORDER BY c.start_date DESC",
        ...
        history.text = if (count == 0) "No previous credit or repayment history available." else
