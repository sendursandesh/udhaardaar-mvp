...
class UdhaardaarApp : Application() {
    private val v5Bg = Color.rgb(246,249,252)
    private val v5Navy = Color.rgb(24,58,92)
    private val v5Blue = Color.rgb(25,111,220)
    private val v5Border = Color.rgb(205,215,225)
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            override fun onActivityResumed(a: Activity) {
                a.window.decorView.post {
                    if (a.javaClass.simpleName.startsWith("V5")) applyV5VisualSystem(a)
    ...
    private fun applyV5VisualSystem(a: Activity) {
        a.window.statusBarColor = v5Bg
        a.window.navigationBarColor = v5Bg
    ...
    private fun style(v: View) {
        when (v) {
            is EditText -> {
                v.setTextColor(v5Navy)
                v.setHintTextColor(Color.rgb(105,120,135))
                v.setPadding(dp(16), dp(10), dp(16), dp(10))
                v.minHeight = dp(52)
                v.background = rounded(Color.WHITE, v5Border, 12)
            }
            is Button -> {
                ...
                v.background = rounded(v5Blue, v5Blue, 12)
                if (v.contentDescription.isNullOrBlank()) v.contentDescription = v.text.toString()
            }
            is Spinner -> {
                v.minimumHeight = dp(52)
                v.background = rounded(Color.WHITE, v5Border, 12)
                v.setPadding(dp(12), 0, dp(12), 0)
            }
            is TextView -> if (v.textSize < 14f) v.setTextSize(14f)
            is ScrollView -> v.setBackgroundColor(v5Bg)
        }
