...
class UdhaardaarApp:Application(){
 ...
 override fun onCreate(){super.onCreate();V7AccountStore.migrateLegacyPreferences(this);V7LegacyEventBridge.install();Thread{migrateLegacyOwner();runCatching{V7LegacyProjectionSync.sync(this)}}.start();registerActivityLifecycleCallbacks(object:ActivityLifecycleCallbacks{override fun onActivityResumed(a:Activity){a.window.addFlags(android.view.WindowManager.LayoutParams.FLAG_SECURE);a.window.decorView.post{applyBars(a);applyMobileLimits(a.window.decorView);installKeyboard(a.window.decorView);style(a.window.decorView)}};override fun onActivityCreated(a:Activity,b:Bundle?)=Unit;override fun onActivityStarted(a:Activity)=Unit;override fun onActivityPaused(a:Activity)=Unit;override fun onActivityStopped(a:Activity)=Unit;override fun onActivitySaveInstanceState(a:Activity,b:Bundle)=Unit;override fun onActivityDestroyed(a:Activity)=Unit})}
 private fun migrateLegacyOwner(){val p=getSharedPreferences("v7_migration_state",MODE_PRIVATE);if(p.getBoolean("legacy_migrated",false))return;runCatching{val u=V32DatabaseHelper(this).userData();if(u!=null){val m=u.mobile.filter(Char::isDigit);if(m.length==10&&V7AccountStore.account(this,m)==null)V7AccountStore.create(this,u.name,m)}};p.edit().putBoolean("legacy_migrated",true).apply()}
 ...
 private fun applyMobileLimits(v:View){if(v is EditText){val h=v.hint?.toString()?.lowercase()?:"";if(h.contains("mobile number")||h.contains("alternate mobile")||h.contains("10-digit mobile")){v.filters=arrayOf(InputFilter.LengthFilter(10));v.keyListener=DigitsKeyListener.getInstance("0123456789")}};if(v is ViewGroup)for(i in 0 until v.childCount)applyMobileLimits(v.getChildAt(i))}
