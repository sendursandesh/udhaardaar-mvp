...
class UdhaardaarApp:Application(){
 ...
 override fun onCreate(){super.onCreate();Thread{migrateLegacyOwner()}.start();registerActivityLifecycleCallbacks(object:ActivityLifecycleCallbacks{override fun onActivityResumed(a:Activity){a.window.decorView.post{applyBars(a);applyMobileLimits(a.window.decorView);installKeyboard(a.window.decorView);style(a.window.decorView)}};override fun onActivityCreated(a:Activity,b:Bundle?)=Unit;override fun onActivityStarted(a:Activity)=Unit;override fun onActivityPaused(a:Activity)=Unit;override fun onActivityStopped(a:Activity)=Unit;override fun onActivitySaveInstanceState(a:Activity,b:Bundle)=Unit;override fun onActivityDestroyed(a:Activity)=Unit})}
