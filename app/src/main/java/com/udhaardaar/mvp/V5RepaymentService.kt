...
class V5RepaymentService(context: Context) {
    ...
    fun confirm(requestId:String,otp:String,expectedOtp:String):Boolean{val r=store.find("repayment_requests",requestId)?:return false;if(r.optString("status")!="COUNTERPARTY_OTP_PENDING"||otp.length!=6||otp!=expectedOtp)return false;r.put("status","CONFIRMED");r.put("consentedAt",System.currentTimeMillis());store.replace("repayment_requests",r);store.add("audit",JSONObject().apply{put("id","AUD-${System.currentTimeMillis()}");put("entityId",requestId);put("event","COUNTERPARTY_OTP_CONSENTED");put("at",System.currentTimeMillis())});return true}
}
