...
object V62Store {
    ...
    const val LIABILITIES = "v62_liabilities"
    ...
    const val CHARGECHECK = "v62_chargecheck"
    const val TRADE_CREDIT = "v62_trade_credit"
    const val FORMAL_CREDIT = "v62_formal_credit"
    const val FUNDING_REQUESTS = "v62_funding_requests"
    const val PEOPLE = "v62_people"
    const val ADDRESSES = "v62_addresses"
    const val BENEFITS = "v62_benefits"
...
object V62Relationships {
    fun createCounterparty(c: Context, relationshipId: String, role: String, name: String, mobile: String = "", pan: String = "", aadhaar: String = "", gstin: String = "") = JSONObject().apply {
        put("id", V62Store.id("CP")); put("role", role); put("name", name.trim()); put("mobile", mobile.trim()); put("pan", pan.trim().uppercase()); put("aadhaar", aadhaar.trim()); put("gstin", gstin.trim().uppercase()); put("transactionContext", relationshipId); put("createdAt", System.currentTimeMillis())
...
object V62MisEngine {
    fun metrics(c: Context): JSONObject {
        ...
        val repayments = V62Store.all(c, V62Store.REPAYMENTS)
        val savings = V62Store.all(c, V62Store.SAVINGS)
        val rel = V62Store.all(c, V62Store.RELATIONSHIPS)
        val liabilities = V62Store.all(c, V62Store.LIABILITIES)
        val idle = currentAssets.filter { it.optBoolean("idle", false) }
        return JSONObject().apply {
            ...
            put("idleFunds", idle.sumOf { it.optDouble("value", 0.0) })
