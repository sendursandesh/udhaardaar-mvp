...
class LoginActivity : AppCompatActivity() {
    ...
    private fun loginPage(){
        val r=base()
        r.addView(text("UDHAARDAAR",30f,teal));r.addView(text("Personal Credit • Assets • Succession",14f));r.addView(text("WELCOME",23f))
        r.addView(text("Choose an option to continue",14f,Color.DKGRAY))
        ...
        add(r,button("CREATE PROFILE / ACCOUNT",navy){registrationPage()},58)
        add(r,button("FORGOT PIN / LOGIN WITH OTP",teal){otpLogin(m.text.toString())},52)
        r.addView(text("Your profile is stored on this device. OTP shown here is a local demo OTP; live SMS delivery requires a configured SMS backend.",12f,Color.DKGRAY))
        show(r)
    }
    ...
    private fun registrationPage(){
        ...
        add(r,button("ADD PROFILE PHOTO",teal){photoTarget=photo;startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply{type="image/*";addCategory(Intent.CATEGORY_OPENABLE);addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION)},501)},52)
        add(r,photo,42)
        ...
        val status=text("OTP not sent",13f,Color.DKGRAY);add(r,status,42)
        add(r,button("SEND OTP",blue){
            val x=m.text.toString();val valid=Regex("^[6-9][0-9]{9}$").matches(x)&&name.text.trim().length>=2&&address.text.trim().length>=5&&p.text.length in 4..6&&p.text.toString()==p2.text.toString()
            if(!valid){toast("Complete name, valid mobile, address and matching 4–6 digit PIN");return@button}
            ...
            AlertDialog.Builder(this).setTitle("OTP SENT").setMessage("Demo OTP: $otp\n\nEnter this 6-digit OTP below to create the profile. Live SMS delivery needs an SMS backend.").setPositiveButton("OK",null).show()
        },58)
        add(r,button("VERIFY OTP + CREATE PROFILE",green){
            if(otpMobile.isBlank()||ob.text.toString()!=otp){ob.error="Enter the latest OTP";toast("Incorrect or missing OTP");return@button}
            val mobile=otpMobile
            prefs.edit().putString("pin_$mobile",p.text.toString()).putString("name_$mobile",name.text.toString().trim()).putString("address_$mobile",address.text.toString().trim()).putString("photo_$mobile",photo.text.toString()).apply()
    ...
    private fun otpLogin(m:String){
        ...
        val i=field("Enter 6-digit OTP",6,true)
        val d=AlertDialog.Builder(this).setTitle("LOGIN OTP SENT").setMessage("Demo OTP: $otp\n\nEnter the OTP below.").setView(i).setNegativeButton("CANCEL",null).setPositiveButton("VERIFY",null).create()
        d.setOnShowListener{d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{if(i.text.toString()!=otp){i.error="Incorrect OTP"}else{d.dismiss();resetPin(m)}}};d.show()
    }
