...
class LoginActivity : AppCompatActivity() {
    ...
    private fun login(){
        val r=shell(); header(r,"Secure sign in","Credit • assets • insurance • records • legacy")
        val m=input("Registered mobile number",true)
        val p=input("4–6 digit PIN",true).apply{inputType=InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD;filters=arrayOf(InputFilter.LengthFilter(6))}
        add(r,m,18); add(r,p)
        add(r,button("LOGIN WITH PIN + OTP"){
            val x=m.text.toString()
            val valid=validMobile(x) && prefs.getString("pin_$x",null)==p.text.toString()
            if(valid) otpDialog("Secure login OTP",x){go(x)}
            else Toast.makeText(this,"Enter a valid registered mobile and PIN",Toast.LENGTH_LONG).show()
        })
        add(r,button("CREATE PROFILE / ACCOUNT",teal){register()},10)
        add(r,button("RESET PIN WITH OTP",Color.rgb(90,105,120)){
            val x=m.text.toString()
            if(validMobile(x)&&prefs.contains("pin_$x")) otpDialog("Reset PIN OTP",x){resetPin(x)}
    ...
    private fun otpDialog(title:String,mobile:String,onVerified:()->Unit){
        ...
        val code=input("Enter 6-digit OTP",true).apply{filters=arrayOf(InputFilter.LengthFilter(6))}
        val d=AlertDialog.Builder(this).setTitle(title).setMessage("Demo OTP: $otp\nProduction SMS gateway is required for live delivery.").setView(code).setNegativeButton("CANCEL",null).setPositiveButton("VERIFY",null).create()
        d.setOnShowListener{d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
            if(code.text.toString()==otp){d.dismiss();onVerified()}else code.error="Incorrect OTP"
        }}
        d.show()
    }
    ...
    private fun resetPin(x:String){
        ...
        AlertDialog.Builder(this).setTitle("Set new PIN").setView(np).setPositiveButton("SAVE"){_,_->
            if(np.text.length in 4..6){prefs.edit().putString("pin_$x",np.text.toString()).apply();Toast.makeText(this,"PIN updated",Toast.LENGTH_LONG).show()}
            else Toast.makeText(this,"PIN must be 4–6 digits",Toast.LENGTH_LONG).show()
        }.setNegativeButton("CANCEL",null).show()
    }
    private fun register(){
        val r=shell(); header(r,"Create your profile","One verified profile for your ArthSaathi records")
        ...
        add(r,n,18); add(r,m); add(r,p)
        add(r,button("VERIFY MOBILE + CREATE ACCOUNT"){
            val x=m.text.toString(); val valid=validMobile(x)&&n.text.toString().trim().length>=2&&p.text.length in 4..6
            if(valid) otpDialog("Verify mobile",x){prefs.edit().putString("pin_$x",p.text.toString()).putString("name_$x",n.text.toString().trim()).apply();go(x)}
            else Toast.makeText(this,"Complete valid profile details",Toast.LENGTH_LONG).show()
        })
        add(r,button("BACK TO SIGN IN",Color.rgb(90,105,120)){login()},10)
        screen(r)
    }
}
