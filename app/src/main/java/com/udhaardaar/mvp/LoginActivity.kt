...
class LoginActivity : AppCompatActivity() {
    ...
    private fun box(fill: Int = Color.WHITE) = GradientDrawable().apply {
        setColor(fill); setStroke(dp(1), Color.rgb(190,210,225)); cornerRadius = dp(14).toFloat()
    }
    private fun label(s: String, size: Float = 15f, c: Int = navy) = TextView(this).apply {
        text = s; textSize = size; setTextColor(c); setPadding(dp(4),dp(3),dp(4),dp(3))
    }
    private fun field(h: String, max: Int = 0, num: Boolean = false) = EditText(this).apply {
        hint = h; textSize = 14f; setSingleLine(true); setPadding(dp(10),dp(4),dp(10),dp(4)); background = box()
    ...
    private fun btn(s: String, c: Int, fn: () -> Unit) = Button(this).apply {
        text = s; isAllCaps = false; textSize = 13f; minHeight = 0; minimumHeight = 0
        setPadding(dp(8),0,dp(8),0); setTextColor(Color.WHITE); background = box(c); setOnClickListener { fn() }
    }
    private fun add(r: LinearLayout, v: View, h: Int = 46) { r.addView(v, LinearLayout.LayoutParams(-1,dp(h)).apply { setMargins(0,dp(2),0,dp(2)) }) }
    private fun root() = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_HORIZONTAL; setPadding(dp(14),dp(12),dp(14),dp(16)); setBackgroundColor(pageBg) }
    private fun show(r: LinearLayout) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(scroll) { v, insets ->
            ...
            v.setPadding(v.paddingLeft,v.paddingTop,v.paddingRight,ime); insets
        }
        setContentView(scroll); window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE)
    }
    ...
    private fun loginPage() {
        val r = root()
        r.addView(label("UDHAARDAAR",28f,teal)); r.addView(label("Personal Credit • Assets • Succession",13f)); r.addView(label("WELCOME",21f)); r.addView(label("Choose an option to continue",13f,Color.DKGRAY))
        ...
        add(r,m); add(r,pin)
        add(r,btn("LOGIN",blue) {
            val x = m.text.toString()
            when {
                !validMobile(x) -> toast("Enter a valid 10-digit mobile number")
                prefs.getString("pin_$x",null) == null -> toast("Account not found — tap CREATE PROFILE below")
                prefs.getString("pin_$x","") != pin.text.toString() -> toast("Incorrect PIN")
                else -> enter(x)
            }
        },48)
        add(r,btn("CREATE PROFILE / ACCOUNT",navy) { registrationPage() },48)
        add(r,btn("FORGOT PIN / LOGIN WITH OTP",teal) { otpLogin(m.text.toString()) },46)
        r.addView(label("Release-candidate OTP is a local test OTP. Live SMS requires a configured SMS gateway/backend.",11f,Color.DKGRAY)); show(r)
    }

    private fun registrationPage() {
        val r = root(); r.addView(label("CREATE PROFILE",22f)); r.addView(label("Enter details, send OTP, verify and create your profile.",12f,Color.DKGRAY))
        ...
        add(r,btn("ADD PROFILE PHOTO",teal) { photoTarget = photo; startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply { type="image/*"; addCategory(Intent.CATEGORY_OPENABLE); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION) },501) },46)
        add(r,photo,38)
        ...
        add(r,p); add(r,p2)
        val ob = field("Enter 6-digit OTP",6,true); ob.visibility = View.GONE; add(r,ob)
        val status = label("OTP not sent",12f,Color.DKGRAY); add(r,status,38)
        add(r,btn("SEND OTP",blue) {
            val x = m.text.toString()
            val ok = validMobile(x) && name.text.toString().trim().length >= 2 && address.text.toString().trim().length >= 5 && p.text.length >= 4 && p.text.length <= 6 && p.text.toString() == p2.text.toString()
        ...
        add(r,btn("BACK TO LOGIN",Color.DKGRAY) { loginPage() },44); show(r)
    }
    ...
    private fun resetPin(m: String) {
        ...
        d.setOnShowListener { d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener { if(np.text.length < 4 || np.text.length > 6) np.error="4–6 digits required" else { prefs.edit().putString("pin_$m",np.text.toString()).apply(); d.dismiss(); enter(m) } } }; d.show()
    }
