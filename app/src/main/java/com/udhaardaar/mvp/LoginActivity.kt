...
class LoginActivity : AppCompatActivity() {
    ...
    private fun loginPage(){
        val r=base();r.addView(logo(90));r.addView(t("Udhaardaar",30f).apply{gravity=Gravity.CENTER});r.addView(t("Your Credit. Your Trust. Our Record.",14f,teal).apply{gravity=Gravity.CENTER});r.addView(Space(this),LinearLayout.LayoutParams(1,dp(16)));r.addView(t("Login",23f));r.addView(t("Registered mobile number + PIN",13f,Color.DKGRAY))
        val m=f("Registered mobile number",10,true);val p=f("4–6 digit PIN",6,true).apply{inputType=InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD};put(r,m);put(r,p)
        put(r,b("LOGIN",blue){val ms=m.text.toString().trim();if(ms.length!=10){toast("Enter a valid 10-digit mobile number");return@b};val saved=prefs.getString("pin_$ms",null);if(saved==null){toast("Account not found. Tap Create account.");return@b};if(saved!=p.text.toString()){toast("Incorrect PIN");return@b};enter(ms)},52)
        val links=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER};links.addView(b("Forgot PIN / OTP",teal){otpLogin(m.text.toString().trim())},LinearLayout.LayoutParams(0,50,1f));links.addView(b("Create account",navy){registrationPage()},LinearLayout.LayoutParams(0,50,1f));r.addView(links,LinearLayout.LayoutParams(-1,dp(54)))
        show(r)
    }

    private fun registrationPage(){
        val r=base();r.addView(logo(70));r.addView(t("Create Account",23f));r.addView(t("Complete your account once. Then use mobile + PIN for login.",13f,Color.DKGRAY))
        val name=f("Full name *");val m=f("Mobile number * (10 digits)",10,true);val address=f("Full address *");val email=f("Email (optional)");val photo=f("Selected photo",400);photo.isFocusable=false;photo.visibility=View.GONE
        put(r,name);put(r,m);put(r,address);put(r,email);put(r,b("ADD PROFILE PHOTO",teal){choosePhoto(photo)},52);put(r,photo,42)
        val p=f("Create 4–6 digit PIN *",6,true).apply{inputType=InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD};val p2=f("Confirm PIN *",6,true).apply{inputType=InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD};put(r,p);put(r,p2)
        val otpBox=f("Enter 6-digit OTP",6,true).apply{visibility=View.GONE};put(r,otpBox)
        put(r,b("SEND OTP",blue){val ms=m.text.toString().trim();if(name.text.trim().length<2||ms.length!=10||address.text.trim().length<5||p.text.length !in 4..6||p.text.toString()!=p2.text.toString()){toast("Complete name, address, mobile and matching PIN");return@b};if(prefs.contains("pin_$ms")){toast("This mobile is already registered. Please login.");return@b};otp=Random.nextInt(100000,1000000).toString();otpMobile=ms;otpBox.visibility=View.VISIBLE;toast("Demo OTP: $otp")},52)
        put(r,b("VERIFY OTP + CREATE ACCOUNT",green){if(otp.isBlank()||otpBox.text.toString()!=otp){toast("Enter the correct OTP");return@b};val ms=otpMobile;if(ms.isBlank()){toast("Send OTP first");return@b};prefs.edit().putString("pin_$ms",p.text.toString()).putString("name_$ms",name.text.toString().trim()).putString("address_$ms",address.text.toString().trim()).putString("email_$ms",email.text.toString().trim()).putString("photo_$ms",photo.text.toString()).apply();enter(ms)},54)
        put(r,b("BACK TO LOGIN",Color.rgb(90,110,125)){loginPage()},50);show(r)
    }
    ...
    private fun enter(ms:String){prefs.edit().putBoolean("logged_in",true).putString("current_mobile",ms).apply();startActivity(Intent(this,V323Activity::class.java).putExtra("authenticated_mobile",ms))}
}
