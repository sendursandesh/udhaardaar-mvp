...
class LoginActivity : AppCompatActivity() {
    ...
    private fun login(){
        ...
        add(r,m,18); add(r,p)
        add(r,button("LOGIN WITH PIN + OTP"){val x=m.text.toString();val valid=validMobile(x)&&prefs.getString("pin_$x",null)==p.text.toString();if(valid)otpDialog("Secure login OTP",x){go(x)}else Toast.makeText(this,"Enter a valid registered mobile and PIN",Toast.LENGTH_LONG).show()})
        add(r,button("CREATE PROFILE / ACCOUNT",teal){register()},10)
        add(r,button("RESET PIN WITH OTP",Color.rgb(90,105,120)){val x=m.text.toString();if(validMobile(x)&&prefs.contains("pin_$x"))otpDialog("Reset PIN OTP",x){resetPin(x)}else Toast.makeText(this,"Enter a registered mobile first",Toast.LENGTH_LONG).show()},10)
        add(r,button("हिन्दी / English",Color.rgb(0,145,135)){languageDialog()},10)
        screen(r)
    }
