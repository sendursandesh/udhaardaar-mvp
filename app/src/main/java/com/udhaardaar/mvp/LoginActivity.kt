...
class LoginActivity : AppCompatActivity() {
    ...
    private val navy=Color.rgb(18,48,76); private val teal=Color.rgb(0,145,135); private val blue=Color.rgb(38,99,235); private val gold=Color.rgb(214,165,26); private val bg=Color.rgb(247,249,252); private val muted=Color.rgb(92,108,124); private val border=Color.rgb(218,226,235)
    ...
    private fun header(r:LinearLayout,title:String,sub:String){
        val h=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(dp(14),dp(14),dp(14),dp(14));background=box()}
        val logo=ImageView(this).apply{setImageResource(R.drawable.udhaardaar_logo);contentDescription="ArthSaathi logo"}
        h.addView(logo,LinearLayout.LayoutParams(dp(70),dp(70)))
        val b=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL}
        b.addView(TextView(this@LoginActivity).apply{text="ARTHSAATHI";textSize=24f;setTextColor(navy);typeface=Typeface.DEFAULT_BOLD})
        b.addView(TextView(this@LoginActivity).apply{text="Navigate Your Financial Journey";textSize=12f;setTextColor(teal);typeface=Typeface.DEFAULT_BOLD})
        b.addView(TextView(this@LoginActivity).apply{text="PLAN  •  PROTECT  •  GROW  •  NOMINATE";textSize=9f;setTextColor(gold);typeface=Typeface.DEFAULT_BOLD})
        b.addView(TextView(this@LoginActivity).apply{text="$title\n$sub";textSize=12f;setTextColor(muted)})
        h.addView(b,LinearLayout.LayoutParams(0,-2,1f).apply{leftMargin=dp(12)})
        r.addView(h)
    }
