...
class V62TTMMActivity : androidx.appcompat.app.AppCompatActivity() {
    ...
    private fun loadGroup() { val g = s.all(V62Store.TTMM_GROUPS).lastOrNull(); if (g != null) { groupId = g.optString("id"); members.clear(); g.optString("members").split("|").filter { it.isNotBlank() }.forEach { members.add(it) } } }
    ...
    private fun render() {
        ...
        add(ArthSaathiV62Design.text(this, "Create a group, record shared expenses using equal/custom/percentage/shares splits, see net balances and record settlements. Every change is retained in the V6.2 ledger.", 10f, ArthSaathiV62Design.MUTED), 8)
        ...
        val payer = ArthSaathiV62Design.input(this, "Paid by — exact member name")
    ...
    private fun saveGroup() {
        ...
        s.replace(V62Store.TTMM_GROUPS, JSONObject().apply { put("id", groupId); put("members", members.joinToString("|")); put("status", "ACTIVE"); put("updatedAt", System.currentTimeMillis()) })
    }
    ...
    private fun record(desc: String, amount: String, payer: String, method: String, raw: String) {
        ...
        s.add(V62Store.TTMM_EXPENSES, JSONObject().apply { put("id", id); put("groupId", groupId); put("description", desc.trim()); put("amount", a); put("payer", payer.trim()); put("splitMethod", method); put("allocations", JSONArray(allocations)); put("members", members.joinToString("|")); put("balanceSummary", summary); put("status", "OPEN"); put("createdAt", System.currentTimeMillis()) })
        V62EventBus.publish(V62Event(V62Events.TTMM_EXPENSE_CHANGED, id))
        Toast.makeText(this, "Expense recorded and balances calculated.", Toast.LENGTH_SHORT).show(); render()
    }
    ...
    private fun balances(): Map<String, Double> {
        ...
        s.all(V62Store.TTMM_EXPENSES).filter { groupId.isBlank() || it.optString("groupId") == groupId }.forEach { e ->
            val ms = e.optString("members").split("|")
            val al = e.optJSONArray("allocations") ?: return@forEach
        ...
        s.all(V62Store.TTMM_SETTLEMENTS).filter { groupId.isBlank() || it.optString("groupId") == groupId }.forEach { x -> out[x.optString("from")] = (out[x.optString("from")] ?: 0.0) + x.optDouble("amount", 0.0); out[x.optString("to")] = (out[x.optString("to")] ?: 0.0) - x.optDouble("amount", 0.0) }
        return out
    }

    private fun settlementDialog() {
        if (members.size < 2) { Toast.makeText(this, "Add members first.", Toast.LENGTH_SHORT).show(); return }
        val from = ArthSaathiV62Design.input(this, "Paid by / owing member")
        val to = ArthSaathiV62Design.input(this, "Paid to / creditor member")
        ...
        AlertDialog.Builder(this).setTitle("Settlement").setView(box).setNegativeButton("CANCEL", null).setPositiveButton("SETTLE") { _, _ ->
            ...
            saveGroup(); val id = V62Store.id("SET"); s.add(V62Store.TTMM_SETTLEMENTS, JSONObject().apply { put("id", id); put("groupId", groupId); put("from", from.text.toString().trim()); put("to", to.text.toString().trim()); put("amount", a); put("status", "SETTLED"); put("createdAt", System.currentTimeMillis()) }); V62EventBus.publish(V62Event(V62Events.TTMM_EXPENSE_CHANGED, id)); render()
        }.show()
    }
}
