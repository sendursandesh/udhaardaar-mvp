...
object V62Integration {
    ...
    fun findCounterparties(c: Context, query: String): List<JSONObject> {
        ...
        return store(c).all(V62Store.COUNTERPARTIES).filter { p ->
            q.isEmpty() || listOf("id", "name", "mobile", "pan", "aadhaar", "gstin", "profileId")
                .any { key -> p.optString(key).lowercase(Locale.getDefault()).contains(q) }
        }
    }
    ...
    fun createCounterparty(
        ...
        if (cleanName.length < 2 || !cleanMobile.matches(Regex("[6-9][0-9]{9}")) || transactionContext.isBlank()) return null
        findCounterparties(c, cleanMobile).firstOrNull()?.let { return it }
        val result = JSONObject().apply {
            ...
            put("transactionContext", transactionContext.trim())
            put("createdBy", currentUserId(c))
            put("createdAt", System.currentTimeMillis())
        }
        store(c).add(V62Store.COUNTERPARTIES, result)
        V62EventBus.publish(V62Event(V62Events.PROFILE_CHANGED, result.optString("id")))
        return result
    }

    fun recordConsent(c: Context, subjectId: String, relationshipId: String, eventType: String, otpVerified: Boolean): JSONObject {
        val o = JSONObject().apply {
            put("id", V62Store.id("CONSENT"))
            put("subjectId", subjectId)
            put("counterpartyId", subjectId)
            put("relationshipId", relationshipId)
            put("eventType", eventType)
            put("granted", otpVerified)
            put("otpVerified", otpVerified)
    ...
    fun recalculateRelationshipOutstanding(c: Context, relationshipId: String): Double {
        ...
        val paidPrincipal = repayments.sumOf { it.optDouble("principal", it.optDouble("amount", 0.0)) }
        val outstanding = (principal - paidPrincipal).coerceAtLeast(0.0)
    ...
    fun score(c: Context, counterpartyId: String): Int {
        ...
        return (750 + if (principal > 0.0) ((repaid / principal) * 120).toInt() else 0 - (overdue / 1000.0).toInt() - rels.count { it.optString("status") == "DEFAULTED" } * 60).coerceIn(300, 900)
    }
    ...
    fun projectTradeCredit(c: Context, relationshipId: String, counterpartyId: String, ocrText: String): JSONObject {
        ...
        val invoice = invoiceRegex.find(ocrText)?.groupValues?.getOrNull(1).orEmpty()
        val amount = amountRegex.find(ocrText)?.groupValues?.getOrNull(1)?.replace(",", "")?.toDoubleOrNull() ?: 0.0
        val gst = gstRegex.find(ocrText)?.groupValues?.getOrNull(1).orEmpty()
        val id = V62Store.id("TRADE")
        val record = JSONObject().apply {
            put("id", id)
            put("relationshipId", relationshipId)
            put("counterpartyId", counterpartyId)
            put("invoiceNumber", invoice)
            put("invoiceAmount", amount)
            put("gstin", gst)
            put("outstandingAmount", amount)
            put("ocrSource", "DOCUMENT_INTELLIGENCE")
            put("sourceSystem", "ARTHSAATHI")
            put("sourceRecordId", relationshipId)
            put("importedAt", System.currentTimeMillis())
            put("requiresReview", true)
        }
        store(c).add(V62Store.TRADE_CREDIT, record)
        V62EventBus.publish(V62Event(V62Events.TRADE_CREDIT_IMPORTED, id))
        return record
    }
}
