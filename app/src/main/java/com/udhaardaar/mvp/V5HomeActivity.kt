...
class V5HomeActivity : AppCompatActivity() {
    ...
    private fun home(){if(!prefs.getBoolean("logged_in",false)){startActivity(Intent(this,LoginActivity::class.java));finish();return};val r=root();header(r,"Personal financial command centre","Credit • repayments • documents • assets • succession");val m=prefs.getString("current_mobile","")?:"";put(r,tv("Welcome, ${prefs.getString("name_$m","User")}",19f));section(r,"CREDIT & OBLIGATIONS",blue)
        tile(r,"₹","Register Informal Credit","Personal • business • trade • advance • rental • given/received",blue){startActivity(Intent(this,V5InformalCreditActivity::class.java))}
        tile(r,"⌕","Borrower Profiles","Search by name/mobile/PAN/Aadhaar/GSTIN/ID • create • history",teal){startActivity(Intent(this,V5BorrowerActivity::class.java))}
        ...
        section(r,"DIGITAL DOCUMENTS",amber);tile(r,"▤","DPN + Guarantor Guarantee","Generate • review • OTP consent • version • audit",amber){documents()};tile(r,"▣","Document & Audit Vault","Documents, consent events and evidence history",amber){documents()}
        section(r,"GUARANTOR",red);tile(r,"♙","Guarantor Profile & Consent","Create/link guarantor • generate guarantee • OTP consent",red){startActivity(Intent(this,V5GuarantorConsentActivity::class.java))}
    ...
    private fun legacy(which:String){startActivity(Intent(this,V4HomeActivity::class.java).putExtra("open",which))}
    private fun documents(){val r=root();header(r,"Digital Documents & Consent","DPN/guarantee are prepared before consent and retained with timestamps");put(r,tv("Required order: final credit terms → DPN/guarantee draft → document review → borrower/guarantor consent → OTP verification → registration → archive.",14f),92);put(r,Button(this).apply{text="GENERATE / REVIEW DOCUMENT PACKET";setOnClickListener{val id="DOC-${System.currentTimeMillis()}";V5LocalStore(this@V5HomeActivity).add("documents",org.json.JSONObject().apply{put("id",id);put("type","DPN_GUARANTEE_PACKET");put("status","DRAFT");put("version",1);put("createdAt",System.currentTimeMillis())});AlertDialog.Builder(this@V5HomeActivity).setTitle("Document packet created").setMessage("Draft packet $id is now versioned and timestamped. It must be reviewed and consented before a protected credit record is treated as completed.").setPositiveButton("OK",null).show()}},58);put(r,Button(this).apply{text="OPEN GUARANTOR CONSENT WORKFLOW";setOnClickListener{startActivity(Intent(this@V5HomeActivity,V5GuarantorConsentActivity::class.java))}},58);put(r,Button(this).apply{text="BACK";setOnClickListener{home()}},50);show(r)}
    private fun simplePage(title:String){val r=root();header(r,title,"V5 workflow");put(r,tv("Workflow entry is retained in V5. Records and audit events use timestamped local persistence and are ready for API-backed implementation.",14f),80);put(r,Button(this).apply{text="BACK TO HOME";setOnClickListener{home()}},52);show(r)}
}
