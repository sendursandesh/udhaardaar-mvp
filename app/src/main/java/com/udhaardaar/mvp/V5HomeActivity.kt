...
class V5HomeActivity : AppCompatActivity() {
    ...
    private fun home(){
        ...
        val welcome=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(12),dp(10),dp(12),dp(10));background=box()};add(welcome,tv("Welcome, ${prefs.getString("name_$m","User")}",18f));add(welcome,tv("Mobile: $m",12f,Color.DKGRAY));add(r,welcome)
        section(r,"PARTY & IDENTITY",teal);add(r,btn("Find / Create Borrower • View Credit History",teal){startActivity(Intent(this@V5HomeActivity,V5PartyActivity::class.java))});add(r,btn("Scan QR • Auto-fill Credit / Invoice",blue){startActivity(Intent(this@V5HomeActivity,V5QrCreditActivity::class.java))});add(r,btn("TTMM • Take The Money, Mate",Color.rgb(155,72,180)){startActivity(Intent(this@V5HomeActivity,V5TtmmActivity::class.java))})
        ...
        add(r,btn("MY PROFILE",teal){simplePage("My Profile","Account owner and privacy controls")});add(r,btn("LOGOUT",Color.rgb(90,110,125)){prefs.edit().putBoolean("logged_in",false).remove("current_mobile").apply();startActivity(Intent(this@V5HomeActivity,LoginActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));finish()});show(r)
    }
    private fun documents(){val r=shell("Digital Documents & Consent","Protected document lifecycle and audit metadata");add(r,tv("DPN / Guarantee lifecycle: Draft → Sent → Viewed → Consent pending → OTP verified → Completed → Archived",14f,navy),90);add(r,btn("OPEN DOCUMENT & CONSENT VAULT",amber){startActivity(Intent(this@V5HomeActivity,V5PartyActivity::class.java))});add(r,btn("BACK TO HOME",navy){home()});show(r)}
    private fun simplePage(title:String,sub:String){val r=shell(title,sub);add(r,tv("This V5 module is part of the unified workflow. Records use timestamps and audit metadata; the detailed workflow is opened from its dedicated feature when available.",14f,navy),100);add(r,btn("BACK TO HOME",navy){home()});show(r)}
}
