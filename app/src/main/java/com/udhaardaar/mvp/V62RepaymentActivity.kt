...
class V62RepaymentActivity : androidx.appcompat.app.AppCompatActivity() {
    ...
    private val LinearLayout.dp: Int get() = (this@V62RepaymentActivity.density * this@V62RepaymentActivity.resources.displayMetrics.density).toInt()
    ...
    private fun render() {
        ...
        else rows.forEach { relationship -> renderRelationship(relationship) }
    ...
    private fun renderRelationship(j: JSONObject) {
        ...
        if (authorised && outstanding > 0) box.addView(ArthSaathiV62Design.button(this, "RECORD REPAYMENT — CONSENT + OTP", ArthSaathiV62Design.GREEN) { record(j, outstanding) })
    ...
    private fun record(j: JSONObject, max: Double) {
        ...
        AlertDialog.Builder(this).setTitle("Repayment consent").setMessage("The repayment is recorded against this relationship. Counterparty consent is required with OTP before the ledger is updated.").setView(w).setNegativeButton("CANCEL", null).setPositiveButton("REQUEST OTP") { _, _ ->
            ...
            if (paid <= 0 || paid > max || interestValue < 0 || interestValue > paid || !validDate(paymentDate)) Toast.makeText(this, "Enter valid repayment, interest and date.", Toast.LENGTH_LONG).show()
    ...
    private fun otp(j: JSONObject, paid: Double, interest: Double, date: String) {
        ...
        dlg.setOnShowListener {
            dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                j.put("outstanding", after)
                j.put("lastRepaymentAmount", paid)
                j.put("lastRepaymentPrincipal", principalPaid)
                j.put("lastRepaymentInterest", interest)
                j.put("lastRepaymentDate", date)
                j.put("consentStatus", "OTP_VERIFIED")
                if (after <= 0.0) j.put("status", "CLOSED")
                store.replace(V62Store.RELATIONSHIPS, j)
                val paymentId = V62Store.id("PAY")
                store.add(V62Store.REPAYMENTS, JSONObject().apply {
                ...
                Toast.makeText(this, "Repayment recorded with consent.", Toast.LENGTH_LONG).show()
                render()
            }
        }
        dlg.show()
