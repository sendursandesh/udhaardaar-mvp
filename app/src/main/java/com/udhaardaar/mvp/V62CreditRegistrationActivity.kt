...
class V62CreditRegistrationActivity:AppCompatActivity(){
    ...
    private fun profile(){
        ...
        add(ArthSaathiV62Design.button(this,"CONTINUE TO TERMS →",ArthSaathiV62Design.GREEN){if(selected==null)Toast.makeText(this,"Select or create the borrower first.",Toast.LENGTH_SHORT).show()else{step=2;render()}},10)
    }
    ...
    private fun terms(){
        ...
        add(ArthSaathiV62Design.button(this,"LINK EXISTING ASSET",ArthSaathiV62Design.BLUE){linkAsset()},6)
    ...
    private fun linkAsset(){
        ...
        val assets=store.all(V62Store.ASSETS).filter{it.optString("ownerUserId")==owner&&it.optString("lifecycleStatus","ACTIVE")=="ACTIVE"}
        if(assets.isEmpty()){Toast.makeText(this,"No asset is available. Add an asset in Asset Vault first.",Toast.LENGTH_LONG).show();return}
        val labels=assets.map{it.optString("Asset name / description").ifBlank{it.optString("type")}}
        AlertDialog.Builder(this).setTitle("Link an existing asset").setItems(labels.toTypedArray()){_,which->
            draft.edit().putString("linkedAssetId",assets[which].optString("id")).putString("linkedAssetName",labels[which]).apply()
            Toast.makeText(this,"Asset linked: ${labels[which]}",Toast.LENGTH_SHORT).show()
