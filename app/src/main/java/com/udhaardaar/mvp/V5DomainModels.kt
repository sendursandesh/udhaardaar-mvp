...
object V5Validation {
 private val pan=Regex("^[A-Z]{5}[0-9]{4}[A-Z]$");private val aadhaar=Regex("^[2-9][0-9]{11}$");private val gstin=Regex("^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$");private val mobile=Regex("^[6-9][0-9]{9}$");private val pin=Regex("^[1-9][0-9]{5}$")
 fun pan(value:String)=value.trim().uppercase().let{it.isEmpty()||pan.matches(it)};fun aadhaar(value:String)=value.isEmpty()||aadhaar.matches(value);fun gstin(value:String)=value.trim().uppercase().let{it.isEmpty()||gstin.matches(it)};fun mobile(value:String)=mobile.matches(value.trim());fun pin(value:String)=pin.matches(value.trim())
}
