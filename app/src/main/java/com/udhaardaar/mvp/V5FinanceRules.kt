...
object V5FinanceRules {
    ...
    fun calculateCreditPlan(principal: Double, roiPercent: Double, periodicity: String, startDate: String, endDate: String, repaymentMethod: String): CreditPlan {
        ...
        } else if (monthlyRate == 0.0) principal else {
    ...
    fun validDate(value: String): Boolean {
        ...
        return f.parse(value, p) != null && p.index == value.length
    }
    fun endDateOnOrAfterStart(startDate: String, endDate: String): Boolean {
        val s = parseDate(startDate) ?: return false
        val e = parseDate(endDate) ?: return false
        return !e.before(s)
    }
    fun monthsBetween(startDate: String, endDate: String): Int {
        val s = parseDate(startDate) ?: return 0
        val e = parseDate(endDate) ?: return 0
        var r = (e.get(Calendar.YEAR) - s.get(Calendar.YEAR)) * 12 + e.get(Calendar.MONTH) - s.get(Calendar.MONTH)
        if (e.get(Calendar.DAY_OF_MONTH) < s.get(Calendar.DAY_OF_MONTH)) r--
    ...
    private fun parseDate(value: String): java.util.Date? {
        ...
        val f = SimpleDateFormat("yyyy-MM-dd", Locale.US).apply { isLenient = false }
        val p = ParsePosition(0)
        return f.parse(value, p)?.takeIf { p.index == value.length }
    }
}
