...
object V5FinanceRules {
    ...
    fun calculateCreditPlan(principal: Double, roiPercent: Double, periodicity: String, startDate: String, endDate: String, repaymentMethod: String): CreditPlan {
        ...
        val totalPayable = if (method.contains("principal") && method.contains("interest")) principal + principal * (roiPercent / 100.0) * (periods / 12.0) else if (monthlyRate == 0.0) principal else { val factor = (1 + monthlyRate).pow(periods); principal * monthlyRate * factor / (factor - 1) * periods }
    ...
    fun endDateOnOrAfterStart(startDate: String, endDate: String): Boolean { val start = parseDate(startDate) ?: return false; val end = parseDate(endDate) ?: return false; return !end.before(start) }
    fun monthsBetween(startDate: String, endDate: String): Int { val start = parseDate(startDate) ?: return 0; val end = parseDate(endDate) ?: return 0; var result = (end.get(Calendar.YEAR) - start.get(Calendar.YEAR)) * 12 + end.get(Calendar.MONTH) - start.get(Calendar.MONTH); if (end.get(Calendar.DAY_OF_MONTH) < start.get(Calendar.DAY_OF_MONTH)) result--; return result.coerceAtLeast(0) }
    private fun parseDate(value: String): java.util.Date? { if (!Regex("^\\d{4}-\\d{2}-\\d{2}$").matches(value)) return null; val format = SimpleDateFormat("yyyy-MM-dd", Locale.US).apply { isLenient = false }; val position = ParsePosition(0); return format.parse(value, position)?.takeIf { position.index == value.length } }
}
