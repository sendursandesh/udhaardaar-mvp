...
class V3DatabaseHelper(context: Context) : SQLiteOpenHelper(context, "udhaardaar_v3.db", null, 7) {
    ...
    /** Creates a credit with both sides explicitly represented. The transaction fails unless both parties exist and are distinct. */
    fun addCreditBetween(lenderPartyId:Long, borrowerPartyId:Long, creditType:String, principalAmount:Double, interestRate:Double, repaymentMethod:String, repaymentAmount:Double, periodicity:String?, startDate:String, endDate:String?, nextDueDate:String?, notes:String?, createdAt:String, graceDays:Int=0, invoiceNumber:String?=null, invoiceUri:String?=null):Long {
        if(lenderPartyId<=0 || borrowerPartyId<=0 || lenderPartyId==borrowerPartyId || principalAmount<=0 || interestRate<0 || repaymentAmount<=0) return -1L
        val db=writableDatabase; db.beginTransaction()
        return try {
            fun exists(id:Long):Boolean=db.rawQuery("SELECT 1 FROM parties WHERE id=? LIMIT 1",arrayOf(id.toString())).use{it.moveToFirst()}
            if(!exists(lenderPartyId)||!exists(borrowerPartyId)) throw IllegalArgumentException("Both lender and borrower must exist")
            val id=db.insertOrThrow("credits",null,ContentValues().apply{put("credit_type",creditType);put("direction","GIVEN");put("principal_amount",principalAmount);put("interest_rate",interestRate);put("repayment_method",repaymentMethod);put("repayment_amount",repaymentAmount);put("periodicity",periodicity);put("start_date",startDate);put("end_date",endDate);put("next_due_date",nextDueDate);put("grace_days",graceDays);put("invoice_number",invoiceNumber);put("invoice_uri",invoiceUri);put("status","ACTIVE");put("notes",notes);put("created_at",createdAt);put("updated_at",createdAt)})
            db.insertOrThrow("credit_parties",null,ContentValues().apply{put("credit_id",id);put("party_id",lenderPartyId);put("role","LENDER")})
            db.insertOrThrow("credit_parties",null,ContentValues().apply{put("credit_id",id);put("party_id",borrowerPartyId);put("role","BORROWER")})
            db.insertOrThrow("credit_access",null,ContentValues().apply{put("credit_id",id);put("party_id",lenderPartyId);put("role","LENDER");put("can_view",1);put("can_repay",1);put("can_edit",1);put("consent_required",1)})
            db.insertOrThrow("credit_access",null,ContentValues().apply{put("credit_id",id);put("party_id",borrowerPartyId);put("role","BORROWER");put("can_view",1);put("can_repay",1);put("can_edit",0);put("consent_required",1)})
            db.insertOrThrow("activity_log",null,ContentValues().apply{put("credit_id",id);put("party_id",lenderPartyId);put("activity_type","CREDIT_CREATED");put("description","Credit relationship registered");put("created_at",createdAt)})
    ...
    fun addCredit(partyId:Long,creditType:String,direction:String,principalAmount:Double,interestRate:Double,repaymentMethod:String,repaymentAmount:Double,periodicity:String?,startDate:String,endDate:String?,nextDueDate:String?,notes:String?,createdAt:String,graceDays:Int=0,invoiceNumber:String?=null,invoiceUri:String?=null):Long {
        return if(direction=="GIVEN") addCreditBetween(partyId, partyId+0L, creditType, principalAmount, interestRate, repaymentMethod, repaymentAmount, periodicity, startDate, endDate, nextDueDate, notes, createdAt, graceDays, invoiceNumber, invoiceUri) else -1L
    }
    ...
    private fun authorised(creditId:Long,partyId:Long,column:String,consentVerified:Boolean)=db.rawQuery("SELECT $column,consent_required FROM credit_access WHERE credit_id=? AND party_id=? LIMIT 1",arrayOf(creditId.toString(),partyId.toString())).use{it.moveToFirst()&&it.getInt(0)==1&&(it.getInt(1)==0||consentVerified)}
