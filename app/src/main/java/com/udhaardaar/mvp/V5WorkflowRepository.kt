...
class V5WorkflowRepository(context:Context){
 ...
 fun saveProfile(p:V5Profile){
  ...
  val duplicate=store.all("profiles").any{it.optString("id")!=p.id&&(
   it.optString("mobile")==mobile || (pan.isNotBlank()&&it.optString("pan")==pan) ||
   (aadhaar.isNotBlank()&&it.optString("aadhaar")==aadhaar) || (gstin.isNotBlank()&&it.optString("gstin")==gstin))}
 ...
 fun saveCredit(id:String,borrower:String,type:String,direction:String,principal:Double,roi:Double,method:String,start:String,end:String,vendor:String="",invoice:String="",sourceQr:String="",consentId:String="",timestamp:Long=System.currentTimeMillis(),totalPayable:Double?=null){
  require(id.isNotBlank()&&borrower.isNotBlank()&&principal>0);require(roi in 0.0..100.0);require(ArthSaathiFinancialRules.validDate(start)&&ArthSaathiFinancialRules.validDate(end));require(end>=start)
  val plan=ArthSaathiFinancialRules.plan(principal,roi,monthsBetween(start,end).coerceIn(1,240),method);val total=totalPayable?:plan.totalPayable;require(total>=principal)
 ...
 private fun monthsBetween(start:String,end:String):Int{val sdf=java.text.SimpleDateFormat("yyyy-MM-dd",java.util.Locale.US).apply{isLenient=false};val a=sdf.parse(start)?:throw IllegalArgumentException("Invalid start date");val b=sdf.parse(end)?:throw IllegalArgumentException("Invalid end date");val ca=java.util.Calendar.getInstance().apply{time=a};val cb=java.util.Calendar.getInstance().apply{time=b};return ((cb.get(java.util.Calendar.YEAR)-ca.get(java.util.Calendar.YEAR))*12+(cb.get(java.util.Calendar.MONTH)-ca.get(java.util.Calendar.MONTH))).coerceAtLeast(1)}
}
